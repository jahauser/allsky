...
#   > for filename in *.FIT; do mv "$filename" "${filename//FIT/fit}"; done
#   
#   crop images
#   > imcopy frame*[bounds] frame*
...
client_script_path = '/Users/jake/Desktop/sextraction/astrometry.net/net/client/client.py'
position_delta = 1.25
...
def get_args():
    ...
    ref_solve_parser = subparsers.add_parser('ref', help='lol')
...
def star_indexed(index_file, line):
    ...
    for line in index_file.readlines():
        star = line.split()
        if ra - position_delta <= float(star[1]) <= ra + position_delta and dec - position_delta <= float(star[2]) <= dec + position_delta:
...
def ref_solve(args):
    ...
    solve_arg = 'python {client} --apikey {key} --downsample 1  --upload {{file}} --wcs {{new_file}}'.format(client=client_script_path, key=key, dec=dec)
